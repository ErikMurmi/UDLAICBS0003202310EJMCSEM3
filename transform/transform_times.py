...
import pandas as pd 
from transform.transforms import date_str_month, get_month_str

def transformTimes(load_id):
    try:
        ...
        if not times_tra.empty:
            for id,nam,nwe,nmo,wnu,mnum,mdes,ecm,cqd,cye \
                ...
                times_dict["end_of_cal_month"].append(date_str_month(ecm)),
                times_dict["calendar_quarter_desc"].append(cqd),
                times_dict["calendar_month_name"].append(get_month_str(mnum)),
        ...
        if times_dict["time_id"]:
            ...
            df_times_tra.to_sql('tra_times',ses_db_stg,if_exists='append',index=False)
        print(df_times_tra)
    except:
        traceback.print_exc()
    finally:
